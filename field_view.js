...
(function(){
  ...
  const toast = (msg, type) => { if (window.toast) { window.toast(msg, type); } else { console[(type==='err'?'error':'log')](msg); } };
  const fmtDate = (iso) => { if (!iso) return ''; const t = iso.split('T')[0]; const [y,m,da] = t.split('-'); if (!y||!m||!da) return esc(iso); return `${da}.${m}.${y}`; };
  const getJSON = (url) => fetch(url, {credentials: 'same-origin'}).then(r=>{ if (!r.ok) throw new Error('HTTP '+r.status); return r.json(); });
  const postForm = (url, dataObj) => { const fd=new FormData(); Object.entries(dataObj||{}).forEach(([k,v])=>fd.append(k, v==null?'':v)); const csrf = (qs('meta[name="csrf-token"]')?.content||''); if (csrf) fd.append('csrf', csrf); return fetch(url,{method:'POST',body:fd,credentials:'same-origin'}).then(r=>{ if(!r.ok) throw new Error('HTTP '+r.status); return r.json();}); };
  const onReady = (fn) => { if (d.readyState === 'loading') d.addEventListener('DOMContentLoaded', fn, {once:true}); else fn(); };

  let lastCode=null, currentField=null;
  ...
  function pulse(){ const p = byId('pulse'); if (!p) return; p.classList.add('active'); setTimeout(()=> p.classList.remove('active'), 800); }
  function preloadCultures(sel){ return getJSON('cultures_list.php').then(j=>{ if (!j || !Array.isArray(j.items)) return; sel.innerHTML = j.items.map(c=>`<option value="${esc(c.id)}">${esc(c.name)}</option>`).join(''); }).catch(()=>{}); }

  function bindEdit(){
    const btn = byId('btnEdit'); if (!btn) return;
    btn.addEventListener('click', ()=>{
      const f = currentField; if (!f) return;
      const form = byId('formEdit'); if (!form) { toast('Не найдена форма редактирования (#formEdit)','err'); return; }
      ...
      set('edit_field_code', f.field_code); set('edit_area_ha', f.area_ha);
      setDate('edit_plow_date', f.plow_date); setDate('edit_sow_date', f.sow_date); setDate('edit_last_water_date', f.last_water_date); setDate('edit_harvest_date', f.harvest_date); setDate('edit_treatment_date', f.treatment_date);
      set('edit_treatment_desc', f.treatment_desc); set('edit_gross_yield', f.gross_yield); set('edit_avg_yield', f.avg_yield); set('edit_notes', f.notes);
      if (window.bootstrap && byId('editModal')){ try { new bootstrap.Modal('#editModal').show(); } catch(e){} } else { const panel = byId('editPanel'); if (panel) panel.style.display = 'block'; }
    });
  }

  function bindAdd(code){
    const btn = byId('btnAdd'); if (!btn) return;
    btn.addEventListener('click', ()=>{
      const form = byId('formEdit'); if (!form) { toast('Не найдена форма редактирования (#formEdit)','err'); return; }
      const sel = byId('edit_culture_id'); if (sel) preloadCultures(sel);
      const set = (id, val) => { const el = byId(id); if (el) el.value = (val ?? ''); };
      const t = byId('editTitle'); if (t) t.textContent = 'Добавить участок '+ (code||'');
      ['edit_field_code','edit_area_ha','edit_plow_date','edit_sow_date','edit_last_water_date','edit_harvest_date','edit_treatment_date','edit_treatment_desc','edit_gross_yield','edit_avg_yield','edit_notes'].forEach(id=>{ const el = byId(id); if (el) el.value=''; });
    ...
    form.addEventListener('submit', (e)=>{
      ...
      postForm('field_save.php', data).then(j=>{
        if (j && j.ok){
          ...
          if (window.bootstrap && byId('editModal')){ try { bootstrap.Modal.getInstance(byId('editModal'))?.hide(); } catch(e){} } else { const panel = byId('editPanel'); if (panel) panel.style.display = 'none'; }
  ...
  function wireForm(){
    ...
    form.addEventListener('submit', (e)=>{
      ...
      if (!/^\d{3,4}$/.test(code)){ toast('Введите 3–4 цифры','err'); return; }
      loadField(code);
    });
  }
  ...
  onReady(()=>{
    ...
    wireEditFormSubmit();
    const codeInput = byId('code');
    if (codeInput && /^\d{3,4}$/.test(codeInput.value||'')){ loadField(codeInput.value); }
  });
})();
