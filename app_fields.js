...
(function(){
  ...
  var USER_ROLE = (document.body && document.body.getAttribute('data-role')) || 'user';
  var toasts = document.querySelector('#toasts');
  function toast(msg,type,timeout){ type=type||'info'; timeout=timeout||2200;
    try{
...
var form=qs('#formCode'), codeInput=qs('#code'), fieldView=qs('#fieldView'), listWrap=qs('#listWrap'), btnList=qs('#btnList'), btnPrint=qs('#btnPrintCard'); var lastCode=null, currentField=null;
  ...
  function renderFieldCard(f, canEdit){
    return '<div class="glass pane">'
      ...
      +   (canEdit ? '<a class="btn primary" href="admin.php?code='+esc(f.field_code)+'">Редактировать</a>' : '')
      + '</div>'
...
function renderEmptyCard(code, canAdd){
  var msg = 'Данные о поле временно отсутствуют.';
  var btn = canAdd ? '<a class="btn primary" href="admin.php?code='+encodeURIComponent(code)+'">Добавить</a>' : '';
  return '<div class="glass pane">'
    + '<div class="row gap" style="justify-content:space-between; align-items:center;">'
    +   '<div>'+msg+'</div>'
    +   btn
  ...
  // Defensive submit interception (capture phase) to prevent full reload even if other handlers fail
  document.addEventListener('submit', function(ev){
    var form = ev.target && ev.target.closest && ev.target.closest('#formCode');
    if (form){
      ev.preventDefault();
      try {
        var input = form.querySelector('#code');
        var code = (input && input.value || '').replace(/\D/g,'').slice(0,4);
        if (!code || code.length<3 || code.length>4){ toast('Введите 3–4 цифры','err'); return; }
        if (typeof loadField === 'function'){ loadField(code); }
      } catch(e){ console.error(e); }
    }
  }, true);

  // Also wire the button click as a fallback
  var btn = qs('#formCode button[type="submit"]');
  if (btn){
    btn.addEventListener('click', function(ev){
      ev.preventDefault();
      var form = qs('#formCode');
      if(!form) return;
      var input = form.querySelector('#code');
      var code = (input && input.value || '').replace(/\D/g,'').slice(0,4);
      if (!code || code.length<3 || code.length>4){ toast('Введите 3–4 цифры','err'); return; }
